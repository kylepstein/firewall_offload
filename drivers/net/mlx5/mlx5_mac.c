...
mlx5_mac_addr_set(struct rte_eth_dev *dev, struct rte_ether_addr *mac_addr)
{
	...
	//FIXME: don't set VF mac, as the mac of VF and VF rep will be the same
	if (priv->representor && priv->representor_id >= 0 && 0) {
...
mlx5_mac_addr_get(struct rte_eth_dev *dev, struct rte_ether_addr *mac_addr)
{
	...
	/* Configuring the VF instead of its representor. */
	if (priv->representor) {
		DRV_LOG(DEBUG, "VF represented by port %u getting primary MAC address",
			dev->data->port_id);
		RTE_ETH_FOREACH_DEV_SIBLING(port_id, dev->data->port_id) {
			priv = rte_eth_devices[port_id].data->dev_private;
			if (priv->master == 1) {
				priv = dev->data->dev_private;
				return mlx5_os_vf_mac_addr_get
				       (priv, mac_addr,
					/* vport_id = rep_id + 1 */
